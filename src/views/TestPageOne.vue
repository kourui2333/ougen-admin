 ...
 * @LastEditTime: 2021-08-30 13:28:55
  ...
  <div class="testPageOne">
    ...
    <el-container style="height: 100%;">
      <el-header>
        ...
        <div class="select">
          <el-select v-model="test.value1" placeholder="按处理情况筛选">
            <el-option
          ...
          <el-select v-model="test.value2" placeholder="按单位,社区筛选">
            <el-option
              v-for="item in test.option2"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            ></el-option>
      ...
      <el-main>
        <el-table
          ...
          :row-class-name="test.tableRowClassName"
        >
          ...
          <el-table-column label="类型">
            <template #default="scope">{{ test.testFromType(scope.row.testFromType) }}</template>
          </el-table-column>
          ...
          <el-table-column prop="outpatientService" label="门诊"></el-table-column>
          <el-table-column label="是否有效">
            <template #default="scope">{{ test.isInUse(scope.row.isInUse) }}</template>
          </el-table-column>
          <el-table-column label="是否处理">
            <template #default="scope">{{ test.isChecked(scope.row.isChecked) }}</template>
          </el-table-column>
